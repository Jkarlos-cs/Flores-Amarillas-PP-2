...
// Preparar los girasoles para que aparezcan después
const sunflowers = Array.from(
  svg.querySelectorAll('use[href="#girasol"]')
);
...
// Mostrar los girasoles uno por uno
setTimeout(() => {
  sunflowers.forEach((sunflower, index) => {
    setTimeout(() => {
      sunflower.style.transition =
    ...
    const heartPaths = allPaths.filter(el => {
      ...
      return style.includes('#9758fc') || style.includes('#9e1fc1');
    });
...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Para las chicas maderistas del grupo:
Un pequeño detalle en amarillo para ustedes.
Que nunca falten la música, los buenos momentos y esas canciones que nos mantienen unidos.
...
function showCredit() {
  ...
  credit.textContent = 'By:JK';

  dedication.appendChild(credit);
